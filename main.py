...
class PongGame:
    ...
    def train_ai(self, genome1, genome2, genome3, genome4, config, draw=False):
        """
        ...
        while run:
            ...
            self.move_ai_paddles(net1, net2, net3, net4)
    ...
    def move_ai_paddles(self, net1, net2, net3, net4):
        """
        Determine where to move the left and the right paddle based on the two 
        neural networks that control them. 
        """
        players = [(self.genome1, net1, self.left_paddle, False, True), (self.genome2, net2, self.right_paddle, False, False),
                   (self.genome3, net3, self.top_paddle, True, True), (self.genome4, net4, self.bottom_paddle, True, False)]
        for (genome, net, paddle, horizontal,left) in players:
            if horizontal:
                output = net.activate(
                (paddle.x, abs(paddle.y - self.ball.y), self.ball.x))
            else:
                output = net.activate(
            ...
            if decision == 0:  # Don't move
                ...
                valid = self.game.move_paddle(horizontal=horizontal, left=left, up=True)
            else:  # Move down
                valid = self.game.move_paddle(horizontal=horizontal, left=left, up=False)
    ...
    def calculate_fitness(self, game_info, duration):
        ...
        self.genome3.fitness += game_info.top_hits + duration
        self.genome4.fitness += game_info.bottom_hits + duration
